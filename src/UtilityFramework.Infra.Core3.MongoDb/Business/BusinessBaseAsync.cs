...
using UtilityFramework.Infra.Core3.MongoDb.Data;
...
namespace UtilityFramework.Infra.Core3.MongoDb.Business
{
    public class BusinessBaseAsync<T> : IBusinessBaseAsync<T> where T : ModelBase
    {
        private static readonly ConcurrentDictionary<Type, (Type[] ReferencedTypes, PropertyInfo[] Properties, Dictionary<string, PropertyInfo> Map)>
    _cache = new();
        ...
        /// <returns></returns>
        public async Task<string> UpdateOneAsync(T entity, bool setLastUpdate = true, bool cascade = false, bool inTask = false)
        {
            try
            {
                if (setLastUpdate)
            ...
            if (cascade)
            {
                await UpdateCascadeAsync(entity, inTask);
            }
        ...
        /// <param name="setLastUpdate"></param>
        /// <returns></returns>
        public async Task<T> UpdateAsync(T entity, bool setLastUpdate = true, bool cascade = false, bool inTask = false)
        {
            try
            {
                if (setLastUpdate)
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public string UpdateOne(T entity, bool cascade = false, bool inTask = false)
        {
            try
            {
                entity.LastUpdate = DateTimeOffset.Now.ToUnixTimeSeconds();
            ...
            if (cascade)
            {
                Task.Run(async () => await UpdateCascadeAsync(entity, inTask).ConfigureAwait(false))
                    .GetAwaiter()
                    .GetResult();
            }
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public T Update(T entity, bool cascade = false, bool inTask = false)
        {
            try
            {
                entity.LastUpdate = DateTimeOffset.Now.ToUnixTimeSeconds();
        ...
        public IMongoQuery ConvertToMongoQuery(FilterDefinition<T> filter)
        {
            ...
            return new QueryDocument(renderedFilter);
        }

        private async Task UpdateCascadeAsync(T entity, bool inTask = false)
        {
            if (inTask)
            {
                _ = Task.Run(async () =>
                {
                    try
                    {
                        await UpdateReferencedCollectionsAsync(entity._id.ToString(), entity);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Erro ao realizar cascade: {ex.InnerException} {ex.Message} {ex.StackTrace}".Trim());
                    }
                });
            }
            else
        ...
        private async Task UpdateReferencedCollectionsAsync(string entityId, T updatedEntity)
        {
            var entityType = typeof(T);
            var database = GetCollectionAsync().Database;

            var (ReferencedTypes, Properties, Map) = _cache.GetOrAdd(entityType, t =>
            {
                var solutionName = Assembly.GetEntryAssembly().GetName().Name?.Split('.')[0];

                var assemblyName = Assembly
                    .GetEntryAssembly()
                    ?.GetReferencedAssemblies()
                    .FirstOrDefault(x => x.Name.Contains($"{solutionName}.Data", StringComparison.OrdinalIgnoreCase));

                var allTypes = Assembly.Load(assemblyName).GetTypes();

                var referencedTypes = allTypes
                    .Where(type => type.GetProperties()
                        .Any(p => p.GetCustomAttribute<ReferenceToAttribute>()?.TargetType == t))
                    .Distinct()
                    .ToArray();

                var properties = t.GetProperties();
                var map = properties.ToDictionary(p => p.Name, p => p);

                return (referencedTypes, properties, map);
            });

            var jsonSettings = new JsonWriterSettings { Indent = false };

            for (int i = 0; i < ReferencedTypes.Length; i++)
            {
                Type referencedType = ReferencedTypes[i];

                var referenceProperties = referencedType.GetProperties()
                    .Where(p => p.GetCustomAttribute<ReferenceToAttribute>()?.TargetType == entityType)
                    .ToArray();

                for (int j = 0; j < referenceProperties.Length; j++)
                {
                    PropertyInfo referenceProperty = referenceProperties[j];
                    var propName = referenceProperty.Name;
                    var referenceClassType = referenceProperty.PropertyType;

                    var referencePropertiesToUpdate = referenceClassType.GetProperties()
                        .Where(p => p.Name != "Id")
                        .ToArray();

                    if (referencePropertiesToUpdate.Length == 0)
                        continue;

                    var collection = database.GetCollection<BsonDocument>(referencedType.Name);
                    var filter = Builders<BsonDocument>.Filter.Eq($"{propName}._id", entityId);

                    var updateBuilder = Builders<BsonDocument>.Update;
                    var updates = new List<UpdateDefinition<BsonDocument>>();

                    for (int k = 0; k < referencePropertiesToUpdate.Length; k++)
                    {
                        PropertyInfo refProp = referencePropertiesToUpdate[k];
                        if (Map.TryGetValue(refProp.Name, out var entityProp) &&
                            entityProp.PropertyType == refProp.PropertyType)
                        {
                            updates.Add(updateBuilder.Set(
                                $"{propName}.{refProp.Name}",
                                BsonValue.Create(entityProp.GetValue(updatedEntity))
                            ));
                        }
                    }
                    ...
                    if (updates.Count > 0)
                    {
                        ...
                        if (result.MatchedCount == 0)
                        {
                            ...
                            var updateJson = combinedUpdate.Render(
                                collection.DocumentSerializer,
                                collection.Settings.SerializerRegistry
                            ).ToJson(jsonSettings);
#if DEBUG
                            Console.WriteLine($"[CASCADE] Table: {collection.CollectionNamespace.CollectionName} | " +
                                            $"Property: {propName} | Query: {filterJson} | Update: {updateJson}");
#endif
                        }
