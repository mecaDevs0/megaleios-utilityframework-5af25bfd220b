...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        public static string ToBrazilianReal(this double value, string culture = "pt-BR")
            => value.ToString("C", CultureInfo.GetCultureInfo(culture));
        ...
        public static string ToBrazilianReal(this double? value, string culture = "pt-BR", string defaultValue = "")
            => value != null ? value.GetValueOrDefault().ToString("C", CultureInfo.GetCultureInfo(culture)) : defaultValue;
        ...
        public static T GetValueByProperty<T>(this object x, string propName)
            => x == null ? default(T) : (T)x.GetValueByProperty(propName);
        ...
        /// </summary>
        public static List<string> GetDiferentFields<T, TY>(this T destination, TY source) where T : class
            where TY : class
        {
            ...
            try
            {
                ...
                for (int i = 0; i < listProperties.Count; i++)
                {
                    ...
                    var targetValue = GetValueByProperty(destination, prop.Name);
            ...
            where TY : class
        {
            ...
            for (int i = 0; i < listProperties.Count; i++)
            {
                ...
                var isClass = prop.PropertyType.IsClass && (prop.IsDefined(typeof(IsClass)) || prop.PropertyType.FullName.ContainsIgnoreCase(slnName));
            ...
            string type = "roadmap", string key = null)
        {
            ...
            await using Stream contentStream = await (await client.SendAsync(request)).Content.ReadAsStreamAsync(),
        ...
        public static AddressViewModel GetInfoLocation(double lat, double lng, string key = "")
        {
            ...
            var response = client.Execute<GmapsResult>(request);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public static string GetEnumMemberValueWithDefault<T>(this T value, string defaultValue = "")
        {
            ...
            if ((type.IsEnum || Nullable.GetUnderlyingType(type)?.IsEnum == true) == false)
        ...
        public static string ToParameterInUrl(this Dictionary<string, string> data)
        {
            var parameters = "";
            ...
            return $"?{parameters}".TrimEnd('&', '?').Trim();
        }
        ...
        public async static Task<ReturnApiViewModel> CallApi(CallApiViewModel model, ILogger logger = null, bool jsonPretty = false, JsonSerializerSettings jsonSettings = null, int? timeout = null)
        {
            try
            {
                ...
                restClient.ClientCertificates = model.Certificates;
                ...
                {
                    restResponse.LogRequest(logger, model.Method, model.Url, restResponse.Content, jsonBody);
                }
        ...
        public static void LogRequest(this RestResponse restResponse, ILogger logger, Method method, string uri, string response, string body = null, bool delimiterStartAndEnd = false)
        {
            try
            {
        ...
        public static void LogRequest(this IRestResponse restResponse, ILogger logger, Method method, string uri, string response, string body = null, bool delimiterStartAndEnd = false)
        {
            try
            {
                var message = new StringBuilder();
        ...
        {
            var response = await CallApi<OpenStreetAddressViewModel>(new()
            {
