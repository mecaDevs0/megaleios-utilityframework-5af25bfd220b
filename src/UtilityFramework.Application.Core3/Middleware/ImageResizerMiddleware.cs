    ...
    public class ImageResizerMiddleware
    {
        struct ResizeParams
        {
            ...
            public override readonly string ToString()
            {
                ...
                _ = sb.Append($"w: {W}, ");
                _ = sb.Append($"h: {H}, ");
                _ = sb.Append($"autorotate: {Autorotate}, ");
                _ = sb.Append($"quality: {Quality}, ");
                _ = sb.Append($"format: {Format}, ");
                _ = sb.Append($"mode: {Mode}");
        ...
        private SKData GetImageData(string imagePath, ResizeParams resizeParams, DateTime lastWriteTimeUtc)
        {
            ...
            // cache the result
            _ = _memoryCache.Set(cacheKey, imageData.ToArray());
        ...
        private static SKBitmap Crop(SKBitmap original, ResizeParams resizeParams)
        {
            ...
            _ = original.ExtractSubset(bitmap, cropRect);
            original.Dispose();
        ...
        private static ResizeParams GetResizeParams(PathString path, IQueryCollection query)
        {
            ...
            if (query.ContainsKey("autorotate"))
                _ = bool.TryParse(query["autorotate"], out resizeParams.Autorotate);

            var quality = 100;
            if (query.ContainsKey("quality"))
                _ = int.TryParse(query["quality"], out quality);
